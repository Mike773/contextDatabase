...
_EXTRACTION_TASK = (
    "Ты анализируешь документ и выделяешь ОБЩУЮ информацию о конкретном "
    "направлении, указанном в «# Контекст».\n\n"
    "ЖЁСТКОЕ ИСКЛЮЧЕНИЕ. Всё перечисленное ниже НЕ извлекается этим "
    "анализатором, для них есть отдельные:\n"
    "- роли, должности, подразделения, иерархии (руководитель/подчинённый) → A5\n"
    "- метрики, KPI, показатели, измерители работы сотрудника → A6\n"
    "- алгоритмы / процессы / регламенты оценки работы сотрудника, порядки "
    "утверждения/расчёта/выплаты, способы интерпретации метрик → A7\n"
    "- любые целевые значения, уровни, пороги, матрицы премий — это часть "
    "метрик/алгоритмов → A6/A7\n\n"
    "Если пункт ПАХНЕТ метрикой / ролью / алгоритмом — пропускай его, даже если "
    "он прямо про направление. Документ, посвящённый оценке сотрудников, для A4 "
    "почти всегда возвращает ПУСТЫЕ массивы.\n\n"
    "Примеры того, что НЕ надо извлекать в этом анализаторе:\n"
    "- \"KPI\" (метрика → A6)\n"
    "- \"целевые значения\", \"уровни KPI\", \"матрица премий\" (метрики/алгоритмы)\n"
    "- \"Начальник отдела\", \"ФД\" (роли → A5)\n"
    "- \"порядок утверждения KPI\", \"оценка выполнения по CRM\" (алгоритмы оценки → A7)\n\n"
    "Примеры того, что можно извлекать (если бы встретилось):\n"
    "- \"Ассортиментная матрица\" как общее понятие направления, не как метрика\n"
    "- \"Направление отвечает за клиентскую базу B2B\" — общая характеристика\n"
    "- \"Внутренняя база клиентов GCRM\" — инструмент направления, не метрика\n\n"
    "КРИТИЧНО: не выдумывай расшифровки аббревиатур. Используй только те, что "
    "перечислены в «# Контекст».\n\n"
    "Для каждого извлечённого элемента ОБЯЗАТЕЛЬНА `quote` из документа.\n\n"
...
_EXTRACTION_STEPS = [
    ...
    "саммари документа и известные direction-level термины.",
    ...
    "Выпиши ПРЕДВАРИТЕЛЬНЫЙ список кандидатов (тем, понятий, утверждений), "
    "которые относятся к этому направлению. Пока не отсеивай — просто набрось "
    "все, что в тексте говорится про направление. Для каждого — цитата.",
    "ФИЛЬТР. Пройдись по списку кандидатов из шага 4 и по КАЖДОМУ отдельно "
    "ответь: это роль / должность / подразделение (→ A5)? это метрика / KPI / "
    "показатель / целевое значение (→ A6)? это алгоритм / процесс / регламент "
    "оценки (→ A7)? Если хотя бы на один вопрос «да» — ВЫБРОСЬ этого кандидата "
    "из списка и напиши «выброшен, относится к A5/A6/A7». Оставь ТОЛЬКО то, "
    "что прошло все три проверки.",
    "Из того, что выжило после шага 5, оформи terms (существительные / "
    "понятия / названия). Для каждого — `name`, `short_description`, "
    "`detailed_description`, `quote`. Сверяйся со списком «Известные "
    "direction-level термины» — если такой же name уже есть, всё равно "
    "приведи его снова с новой цитатой. Пустой массив — это **ожидаемый** "
    "результат для документов про оценку сотрудников.",
    "Из того, что выжило после шага 5, оформи claims (утверждения-"
    "предложения). `short_description` (1 предложение), `detailed_description` "
    "(понятно без контекста!), `quote`. Пустой массив — снова нормально.",
    "Напиши `reasoning` (2–3 предложения): что выжило после фильтра, что "
    "было выброшено и почему.",
